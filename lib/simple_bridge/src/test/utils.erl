...
make_inets_get_bridge() ->
    ...
    {ok, [InetsRequest] } = file:consult(code:priv_dir(simple_bridge) ++ "/inets_get_request_data"),
...
make_inets_post_bridge() ->
    % Construct the inets bridge...
    {ok, [InetsRequest] } = file:consult(code:priv_dir(simple_bridge) ++ "/inets_post_request_data"),
    {ok, Socket} = gen_tcp:connect(?PEER_IP, ?PEER_PORT, []),
    InetsRequest1 = inject_socket(InetsRequest, Socket),
    InetsBridge = simple_bridge:make_request(inets_request_bridge, InetsRequest-2),
    InetsBridge.
